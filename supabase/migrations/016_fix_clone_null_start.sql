-- 016_fix_clone_null_start.sql
-- Fix clone_project_as_demo failing when source project has no start_date set.
-- projects.start_date is nullable (UI never sets it on creation), so
-- v_offset = v_new_start - NULL = NULL, which then propagates to
-- milestones.planned_date (NOT NULL) and raises a constraint violation.
-- Fix: fall back to v_new_start when start_date is NULL, giving offset = 0
-- (dates are not shifted, which is the safest default for undated projects).
...
begin
  ...
  -- If source has no start_date, offset = 0 (dates copied as-is).
  v_new_start := date_trunc('week', current_date)::date;
  v_offset    := v_new_start - coalesce(v_source.start_date, v_new_start);

-- ─────────────────────────────────────────────────────────────────────────────
-- 018_recalc_bypass_committed_at.sql
--
-- Problem: _calc_daily_ppc filters out tasks where committed_at > p_date.
-- This is correct for auto-calc (prevents retroactively inflating past PPC),
-- but it breaks manual recalc (p_is_auto = false) when the week is committed
-- on or after the calc date — e.g. committing week 16 on Wednesday then
-- manually recalcing for Tuesday returns 0 planned because all tasks have
-- committed_at = today > yesterday.
--
-- Fix: bypass the committed_at guard when p_is_auto = false.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  with task_seg as (
    ...
    where pt.project_id    = p_project_id
      ...
      -- For auto-calc: only count tasks committed on or before p_date
      -- (prevents retroactive inflation of historical PPC records).
      -- For manual recalc: bypass this guard — the user explicitly wants
      -- the current committed state, regardless of when they pressed Commit.
...
grant execute on function _calc_daily_ppc(uuid, date, boolean) to authenticated;
